...
static void test_forEachSquare_visits_all_64(void) {
  int count = 0;
  ChessIterator::forEachSquare(board, [&](int r, int c, char) { ++count; });
...
static void test_forEachSquare_visits_occupied_and_empty(void) {
  ...
  ChessIterator::forEachSquare(board, [&](int, int, char p) {
    if (p != ' ')
      ++occupied;
    else
      ++empty;
...
static void test_somePiece_returns_false_empty_board(void) {
  bool found = ChessIterator::somePiece(board, [](int, int, char) {
...
static void test_somePiece_finds_matching_piece(void) {
  ...
  bool found = ChessIterator::somePiece(
      board, [](int, int, char p) { return p == 'Q'; });
...
static void test_somePiece_stops_early(void) {
  ...
  placePiece(board, 'P', "c2");

  int visited = 0;
  ChessIterator::somePiece(board, [&](int, int, char) {
    ++visited;
...
static void test_findPiece_not_found(void) {
  int positions[2][2];
  int count = ChessIterator::findPiece(board, 'K', 'w', positions, 2);
  TEST_ASSERT_EQUAL(0, count);
}
...
static void test_findPiece_multiple(void) {
  ...
  int positions[4][2];
  int count = ChessIterator::findPiece(board, 'R', 'w', positions, 4);
...
static void test_findPiece_respects_max(void) {
  ...
  int positions[2][2];
  int count = ChessIterator::findPiece(board, 'P', 'w', positions, 2);
  TEST_ASSERT_EQUAL(2, count);
}

static void test_findPiece_distinguishes_color(void) {
  placePiece(board, 'r', "a8");  // black rook
  placePiece(board, 'R', "a1");  // white rook

  int positions[2][2];
  int countWhite = ChessIterator::findPiece(board, 'R', 'w', positions, 2);
  TEST_ASSERT_EQUAL(1, countWhite);

  int countBlack = ChessIterator::findPiece(board, 'R', 'b', positions, 2);
