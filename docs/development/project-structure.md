...
| `src/chess_attacks.h/.cpp` | `ChessAttacks` namespace: precomputed leaper tables (`KNIGHT_ATTACKS[64]`, `KING_ATTACKS[64]`, `PAWN_ATTACKS[2][64]`, ~2.5 KiB), classical ray functions (`rookAttacks`, `bishopAttacks`, `queenAttacks`), `initAttacks()` initializer. |
...
| `src/chess_evaluation.h/.cpp` | `ChessEval` namespace: position evaluation — `evaluatePosition(bb)` combines material counting with piece-square tables (PSTs, 384 bytes constexpr). Returns score in centipawns. Iterates piece bitboards via `popLsb`; mirrors black via `sq ^ 56`. |
...
└── test_core/
    ...
    └── test_chess_bitboard.cpp     ChessBitboard + ChessAttacks: square mapping roundtrip, bit ops, leaper tables, slider rays, BitboardSet mutations, format stability
```
