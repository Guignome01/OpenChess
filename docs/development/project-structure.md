...
| `src/chess_evaluation.h/.cpp` | `ChessEval` namespace: position evaluation — `evaluatePosition(bb)` combines material counting with piece-square tables (PSTs, 384 bytes constexpr). Returns score in pawns. Iterates piece bitboards via `popLsb`; mirrors black via `sq ^ 56`. |
