...
| `src/chess_utils.h/.cpp` | `ChessUtils` namespace: piece color detection, material evaluation, board queries (`isValidSquare`), piece construction (`makePiece`, `isValidPromotionChar`), castling rights string formatting/parsing, and inline helper functions (`getPieceColor`, `isWhitePiece`, `isBlackPiece`, `isEnPassantMove`, `getEnPassantCapturedPawnRow`, `isCastlingMove`, `colorName`, `opponentColor`, `squareName`, `fileChar`, `rankChar`, `fileIndex`, `rankIndex`). |
