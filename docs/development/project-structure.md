...
| `src/chess_piece.h` | `ChessPiece` namespace (header-only): type-safe piece representation. Piece/color/type extraction (`pieceType`, `pieceColor`), construction (`makePiece`), predicates (`isWhite`, `isBlack`, `isEmpty`, `isPromotion`), color-derived constants (`pawnDirection`, `homeRow`, `promotionRow`), FEN char conversion (`charToPiece`, `pieceToChar`), Zobrist index. All `constexpr`. |
| `src/chess_rules.h/.cpp` | `ChessRules` class: stateless, all-static chess logic. Move generation (fills `MoveList&`), legal move filtering, check/checkmate/stalemate, castling validation, en passant. Position-dependent state (castling rights, en passant target) is passed in via `const PositionState&`. |
...
| `src/chess_utils.h/.cpp` | `ChessUtils` namespace: board-level helpers — material evaluation (`evaluatePosition`), board queries (`isValidSquare`), coordinate helpers (`squareName`, `fileChar`, `rankChar`, `fileIndex`, `rankIndex`), validation (`isValidPromotionChar`), castling rights formatting/parsing, special-move analysis structs (`checkEnPassant`, `checkCastling`, `updateCastlingRights`), `applyBoardTransform`, `boardToText`, `gameResultName`. |
| `src/chess_iterator.h` | `ChessIterator` namespace (header-only): board iteration helpers built on a single internal `detail::forEachSquareUntil` primitive. Public API: `forEachSquare(board, fn)`, `forEachPiece(board, fn)`, `somePiece(board, fn)`, `findPiece(board, type, color, positions, max)`. |
