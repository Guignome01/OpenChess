...
| `src/chess_movegen.h/.cpp` | `ChessMovegen` namespace: precomputed leaper tables (`KNIGHT_ATTACKS[64]`, `KING_ATTACKS[64]`, `PAWN_ATTACKS[2][64]`, ~2.5 KiB), O(1) slider functions (`rookAttacks` via first-rank table + Hyperbola Quintessence, `bishopAttacks` via HQ on diagonal masks, `queenAttacks` = rook+bishop), x-ray functions (`xrayRookAttacks`, `xrayBishopAttacks`), ray geometry (`rayBetween`, `lineBB`), `AttackInfo` struct + `computeAttackInfo(bb)` (per-piece-type/color attack maps for evaluation/search), `initAttacks()` initializer. |
...
└── test_core/
    ...
    ├── test_chess_movegen.cpp      ChessMovegen: leaper tables, slider attacks (+ bulk reference cross-check), x-ray attacks, geometry rays, AttackInfo
