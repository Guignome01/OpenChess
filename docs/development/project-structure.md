...
| `src/chess_evaluation.h/.cpp` | `ChessEval` namespace: position evaluation — `evaluatePosition(bb)` combines material counting, piece-square tables (PSTs, 384 bytes constexpr), and pawn structure analysis (passed/isolated/doubled/backward pawns, connected passers via `ChessPieces` masks). Returns score in centipawns. Iterates piece bitboards via `popLsb`; mirrors black via `sq ^ 56`. |
