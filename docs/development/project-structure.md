...
| File | Purpose |
|------|---------|
| `game/game_mode.h/.cpp` | Abstract base class for all game modes. Holds a `ChessGame*` that orchestrates chess state, recording, and observer notification. Implements shared logic: `tryPlayerMove()`, `applyMove()` (delegates to `ChessGame::makeMove()`), `applyUCIMove()` (parses UCI string + delegates to `applyMove()`), `waitForBoardSetup()`, `tryResumeGame()`, resign gesture handling, and LED feedback helpers. No longer includes `rules.h` directly — all chess queries go through the game orchestrator. |
...
| `src/utils.h/.cpp` | `ChessUtils` namespace: FEN ↔ board conversion, piece color detection, material evaluation, and inline helper functions (`getPieceColor`, `isWhitePiece`, `isBlackPiece`, `isEnPassantMove`, `getEnPassantCapturedPawnRow`, `isCastlingMove`, `colorName`, `opponentColor`, `squareName`). |
