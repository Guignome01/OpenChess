...
All chess-state mutations (`makeMove`, `loadFEN`, `endGame`, `newGame`) flow through this class. `makeMove()` atomically: validates via board, applies (the board's `detectGameEnd()` handles all end conditions including threefold repetition internally), records in history with full `MoveEntry` metadata (addMove handles both in-memory log and persistence automatically), auto-saves recording on game-end, fires callback, and notifies observer. `undoMove()` and `redoMove()` step the history cursor and call `ChessBoard::reverseMove()`/`applyMoveEntry()` respectively. `canUndo()` and `canRedo()` delegate to `ChessHistory`.
