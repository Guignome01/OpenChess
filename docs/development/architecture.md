...
- **Move generation** — `getPossibleMoves(bb, mailbox, row, col, state, moves)` fills a `MoveList&` with all legal moves for a piece at a given position. Internally generates pseudo-legal moves per piece type using bitboard attack tables and ray loops, then filters out any move that would leave the player's king in check (`leavesInCheck()` — copies `BitboardSet` ~120 bytes, applies move, checks king square).
