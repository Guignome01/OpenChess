...
- **Game state checks** — `isCheck(board, color)`, `isCheckmate(board, color, flags)`, `isStalemate(board, color, flags)`, `hasAnyLegalMove(board, color, flags)`, `isPawnPromotion(piece, row)`, `isSquareUnderAttack(board, row, col, defendingColor)`.
