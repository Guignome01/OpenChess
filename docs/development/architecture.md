...
`ChessEval` (in `lib/core/`, `chess_evaluation.h/cpp`) is a namespace providing position evaluation via `evaluatePosition(const BitboardSet& bb)`. Returns a score in pawns (positive = White advantage). Combines material counting (P=1, N=3, B=3, R=5, Q=9) with piece-square tables (PSTs) — 6 tables of 64 centipawn bonuses in LERF order, based on the simplified evaluation function (CPW / Tomasz Michniewski). Black pieces mirror via `sq ^ 56`. Iterates piece bitboards via `popLsb` — no mailbox needed.
...
- `evaluatePosition(bb)` — material + piece-square table score in pawns
