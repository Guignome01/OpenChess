...
All chess-state mutations (`makeMove`, `loadFEN`, `endGame`, `newGame`) flow through this class. `ChessGame` is the sole owner of lifecycle state: `gameOver_`, `gameResult_`, and `winnerColor_`. The board has no concept of game-over — `endGame()`, `isGameOver()`, `gameResult()`, and `winnerColor()` exist only on `ChessGame`. `makeMove()` atomically: validates via board, applies (the board's private `detectGameEnd()` handles all end conditions including threefold repetition, returning results via `MoveResult`), reads `MoveResult` to update lifecycle state, records in history with full `MoveEntry` metadata (addMove handles both in-memory log and persistence automatically), auto-saves recording on game-end, fires callback, and notifies observer. `undoMove()` and `redoMove()` step the history cursor and call `ChessBoard::reverseMove()`/`applyMoveEntry()` respectively; `undoMove()` clears lifecycle state. `canUndo()` and `canRedo()` delegate to `ChessHistory`.
