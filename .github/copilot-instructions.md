...
### Key Components
**Core library** (`lib/core/`): `ChessGame` (orchestrator, lifecycle owner), `ChessBoard` (position container, move execution, game-end detection), `ChessRules` (stateless static logic), `ChessUtils` (utility namespace), `ChessFEN` (FEN parse/serialize), `ChessNotation` (coordinate/SAN/LAN conversion), `ChessHistory` (move log + persistent recording), `types.h` (shared structs/enums), `IGameStorage`/`IGameObserver`/`ILogger` (DI interfaces). Full API details in `.github/instructions/core-library.instructions.md` (auto-loaded for `lib/core/`).
