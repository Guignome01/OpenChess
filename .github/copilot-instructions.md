...
- **`ChessUtils`** (`lib/core/`) — namespace with utility functions: piece color detection, material evaluation, board queries (`findPiece`, `isValidSquare`), piece construction (`makePiece`, `isValidPromotionChar`), castling rights string formatting/parsing, inline helper functions (`opponentColor`, `squareName`, `colorName`, `fileChar`, `rankChar`, `fileIndex`, `rankIndex`), and struct-returning analyzers (`checkEnPassant()` → `EnPassantInfo`, `checkCastling()` → `CastlingInfo`, `updateCastlingRights()`). Uses `std::string` (not Arduino `String`). Firmware call sites bridge with `.c_str()` / `std::string()`.
- **`ChessFEN`** (`lib/core/`) — namespace with FEN string handling: `boardToFEN()` (board array → FEN string), `fenToBoard()` (FEN string → board array + state), `validateFEN()` (format validation: rank structure, piece chars, turn, castling, en passant, clocks). All functions are pure — board and state passed in as parameters.
...
- **`ChessGame`** (`lib/core/`) — central game orchestrator and sole owner of game lifecycle state (`gameOver_`, `gameResult_`, `winnerColor_`). Composes `ChessBoard`, `ChessHistory`, and optionally `IGameObserver`. Constructor: `(IGameStorage*, IGameObserver*, ILogger*)`. All chess-state mutations (`makeMove`, `loadFEN`, `endGame`, `newGame`) flow through this class. `makeMove()` reads `MoveResult` from Board to update lifecycle state; `endGame()` sets lifecycle state directly (guarded against double-calls). Handles move history recording (automatic via `ChessHistory::addMove()`), observer notification, batching, and undo/redo (`undoMove()`, `redoMove()`, `canUndo()`, `canRedo()` — step the history cursor and update the board via `reverseMove()`/`applyMoveEntry()`; `undoMove()` clears lifecycle state). Navigation queries: `currentMoveIndex()` (0 = start position), `moveCount()`, `getHistory(out[], maxMoves, MoveFormat)` (populates array with formatted move strings — COORDINATE, SAN, or LAN). Exposes the full public API: lifecycle methods (`isGameOver()`, `gameResult()`, `winnerColor()`, `endGame()`), board queries, draw detection (`isDraw()`, `isThreefoldRepetition()`), history access, and notation convenience methods (`makeMove(string)`, `toCoordinate()`, `parseCoordinate()`, `parseMove()`). Firmware game modes interact with chess state exclusively through `ChessGame*`.
...
The `lib/core/` library contains all natively-compilable chess logic (`ChessGame`, `ChessBoard`, `ChessHistory`, `ChessRules`, `ChessNotation`, `ChessUtils`, `ChessFEN`, types, Zobrist keys) with zero Arduino dependencies. Tests include `lib/core/` headers directly. Shared test utilities live in `test/test_helpers.h`.
...
- **`test_chess_utils.cpp`** — material evaluation, piece color helpers, 50-move rule, legal move queries, castling rights strings.
...
- **`test_chess_board.cpp`** — ChessBoard: new game, moves, captures, en passant, castling, promotion, check, checkmate, stalemate, 50-move draw, insufficient material, threefold repetition, FEN loading, encode/decode, API queries, reverseMove (normal/capture/en passant/castling/promotion), applyMoveEntry.
- **`test_chess_game.cpp`** — ChessGame: lifecycle (endGame, isGameOver, move rejection after game-over, checkmate sets game-over, undo/newGame/loadFEN reset game-over), draw detection, observer notification/batching, history integration, undo/redo, getHistory (coordinate/SAN/LAN).
