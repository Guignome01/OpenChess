...
### Key Components
**Core library** (`lib/core/`): `ChessGame` (orchestrator, lifecycle owner), `ChessBoard` (position container, move execution, game-end detection), `ChessRules` (stateless static logic), `ChessPiece` (type-safe piece representation, all constexpr), `ChessUtils` (board-level utility namespace), `ChessIterator` (board iteration: `forEachSquare`, `forEachPiece`, `somePiece`, `findPiece`), `ChessHash` (constexpr Zobrist key generation + board hashing), `ChessFEN` (FEN parse/serialize), `ChessNotation` (coordinate/SAN/LAN conversion), `ChessHistory` (move log + persistent recording), `types.h` (shared structs/enums), `IGameStorage`/`IGameObserver`/`ILogger` (DI interfaces). Full API details in `.github/instructions/core-library.instructions.md` (auto-loaded for `lib/core/`).
