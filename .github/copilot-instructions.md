...
### Key Components
**Core library** (`lib/core/`): `ChessGame` (orchestrator, lifecycle owner), `ChessBoard` (position container with `BitboardSet` + `Piece mailbox[64]`, move execution, game-end detection), `ChessBitboard` (bitboard types, LERF square mapping, `BitboardSet` struct), `ChessMovegen` (precomputed leaper tables + classical ray functions + attacked-by maps), `ChessPiece` (type-safe piece representation, all constexpr) + `ChessPieces` (pawn-structure masks and helper queries) — both in `chess_pieces.h`, `ChessRules` (stateless static logic, bitboard-based attack detection), `ChessUtils` (board-level utility namespace), `ChessEval` (position evaluation: material + piece-square tables + pawn structure), `ChessIterator` (bitboard-based iteration: `forEachSquare`, `forEachPiece`, `somePiece`, `findPiece`), `ChessHash` (constexpr Zobrist key generation + incremental hashing), `ChessFEN` (FEN parse/serialize), `ChessNotation` (coordinate/SAN/LAN conversion), `ChessHistory` (move log + persistent recording), `types.h` (shared structs/enums), `IGameStorage`/`IGameObserver`/`ILogger` (DI interfaces). Full API details in `.github/instructions/core-library.instructions.md` (auto-loaded for `lib/core/`).
