...
- **Physical gesture** — on the current player's turn, lift your king and hold it off its square for 3 seconds. The origin square transitions from valid-move highlights to dim orange (33%). Return the king, then perform 2 quick lift-and-return cycles within 1 second each — orange brightens to 66% then 100%. A `boardConfirm()` dialog follows; on confirm the opponent wins with a firework animation.
...
- **State machine** — `ResignPhase` enum (`RESIGN_IDLE`, `RESIGN_GESTURING`). The 3s-hold detection lives inside `tryPlayerMove()`; on put-back it transitions to `RESIGN_GESTURING`. `continueResignGesture()` is a blocking loop for the 2 remaining quick lifts.
...
- **LED helpers** — `showResignProgress(row, col, level, clearFirst)` and `clearResignFeedback(row, col)` encapsulate the LedGuard + setSquareLED + showLEDs pattern for resign feedback. `showIllegalMoveFeedback(row, col)` wraps the red-blink on timeout.
