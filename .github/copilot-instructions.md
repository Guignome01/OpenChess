...
- **`ChessEngine`** — pure chess logic: move generation, validation, check/checkmate/stalemate, castling rights, en passant, 50-move rule, threefold repetition detection via Zobrist hashing (`zobrist_keys.h` — pre-computed tables in PROGMEM, ~6.2KB flash). No hardware dependencies.
...
- **`MoveHistory`** — LittleFS-based game recording and resume system. Binary format: 16-byte packed `GameHeader` + 2-byte UCI-encoded moves + FEN table. Live game persistence (`/games/live.bin`, `/games/live_fen.bin`) enables crash recovery. Storage limits: max 50 games, 80% LittleFS capacity. Provides `GameResult` enum (checkmate, stalemate, draw_50, draw_3fold, resignation), `GameModeCode` enum, NTP timestamps, JSON API for web UI game list, and `replayIntoGame()` for game resume. `MoveHistory` is a `friend` of `ChessGame` for replay access to `applyMove`/`advanceTurn`.
- **`SensorTest`** — standalone sensor testing mode (does not inherit `ChessGame`). Follows the same `begin()`/`update()`/`isComplete()` lifecycle. Tracks visited squares, completes when all 64 are visited.
...
### Utilities
- **`grid_scan_test.cpp`** — standalone hardware debugging utility at the repo root (not compiled in normal builds). Tests shift register column scanning + row GPIO reads.

## Patterns & Conventions

### Code Style
C++ formatting is enforced via `.clang-format` (Google style base, no column limit). Run clang-format before committing.
...
### External APIs
- **Stockfish**: HTTPS calls to `stockfish.online` — see `StockfishAPI` for request/response handling. 8 difficulty presets in `stockfish_settings.h` (beginner→master, depths 3–17, scaled timeouts). `StockfishSettings::fromLevel(int)` factory selects by 1-based level. `BotConfig` bundles `StockfishSettings` + player color.
- **Lichess**: HTTPS to `lichess.org` — polling-based game stream, move submission. Token stored in NVS.

### Game Mode Lifecycle
Each mode follows: `begin()` (setup board, wait for pieces) → `update()` (poll sensors, process moves) → `isGameOver()` triggers return to selection screen. The main loop in `main.cpp` uses `MenuNavigator` to push/pop menus (game selection → bot difficulty → bot color), then calls `initializeSelectedMode()` once a final mode is chosen. The web UI can also trigger game selection via `wifiManager.getSelectedGameMode()`, bypassing the physical menu.

### Game Resume
On boot, `checkForResumableGame()` detects a live game on LittleFS. A `boardConfirm()` dialog with a mode-colored blink indicator asks the player to confirm resume or discard. On confirm, `MoveHistory::replayIntoGame()` restores the game state by finding the last FEN snapshot and replaying subsequent UCI moves. Supports `ChessMoves` and `Bot` modes (not Lichess). A `resumingGame` flag prevents the live game from being discarded during mode initialization.

### Web Board Editing
The web UI can push FEN-based board edits. `WiFiManagerESP32` stores a pending FEN string; the main `loop()` checks via `getPendingBoardEdit()` and applies it to the active game via `setBoardStateFromFEN()`.

### NTP Time Sync
`configTime(0, 0, "pool.ntp.org", "time.nist.gov")` is called once in `setup()`. Non-blocking — NTP resolves in the background and provides Unix timestamps for `MoveHistory` game headers.
...
- **Documentation** — keep `README.md`, `.claude/CLAUDE.md`, `.github/copilot-instructions.md`, and the `docs/` folder in sync with architectural and behavioral changes. When a code change affects architecture, public APIs, endpoints, configuration, build pipeline, file structure, or engineering conventions, update the relevant `docs/` pages in the same change. Do not defer documentation to a follow-up. Specific triggers: new/changed API endpoints → update `docs/development/api.md`; new/removed source files → update `docs/development/project-structure.md`; build or dependency changes → update `docs/development/installation.md`; new LED behaviors, menu changes, or physical interaction changes → update the relevant `docs/guides/` file. When adding a new subsystem, document its purpose, lifecycle, and integration points. Code comments explain *why*, not *what*.